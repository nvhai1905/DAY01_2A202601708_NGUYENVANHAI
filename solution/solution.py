...
def chat_with_system_prompt(
    ...
    """
    Gọi API với MESSAGES gồm 2 phần:
    - system prompt: định hình vai trò/persona của model
    - user prompt: câu hỏi của người dùng

    Returns:
        Tuple (response_text: str, latency_seconds: float).
    """
    # Phải import bên trong hàm để pytest có thể mock OpenAI.
    from openai import OpenAI

    # OPENAI_BASE_URL được dùng khi chạy Gemini hoặc NVIDIA NIM.
    # Nếu không có biến này, SDK sẽ dùng endpoint OpenAI mặc định.
    client = OpenAI(
        api_key=os.getenv("OPENAI_API_KEY"),
        base_url=os.getenv("OPENAI_BASE_URL") or None,
    )
...
def count_tokens(
    ...
    Nếu tiktoken không nhận diện được model hoặc xảy ra lỗi,
    sử dụng phương pháp ước lượng:
        1 token ≈ 4 ký tự.

    Args:
        text:  Đoạn text cần đếm.
        model: Model dùng để chọn encoding.

    Returns:
        Số token dưới dạng int.
    """
    try:
        import tiktoken
        ...
        # Gemini và một số model khác không có encoding tương ứng
        # trong tiktoken nên sẽ sử dụng fallback này.
...
def estimate_cost(
    ...
    """
    Tính chi phí một lượt gọi API dựa trên số token của prompt và response.
    ...
    # Dùng giá đúng của model nếu có trong bảng.
    # Nếu model không tồn tại trong bảng giá thì dùng gpt-4o
    # làm giá tham chiếu.
    pricing = PRICING_PER_1K_TOKENS.get(
        model,
        PRICING_PER_1K_TOKENS["gpt-4o"],
...
def streaming_chatbot() -> None:
    ...
    Hành vi:
        - Stream nội dung ngay khi các chunk được sinh ra.
        - Duy trì tối đa 4 lượt hội thoại gần nhất, tương ứng 8 message.
        - Gõ quit, exit hoặc bye để thoát.
    """
    # Import bên trong hàm để pytest có thể mock OpenAI.
    from openai import OpenAI
    ...
    while True:
        ...
        # Chuẩn hóa chuỗi để thoát không phân biệt chữ hoa/chữ thường
        # và không bị ảnh hưởng bởi khoảng trắng.
        if user_msg.strip().lower() in ("quit", "exit", "bye"):
            break

        # Tin nhắn hiện tại được gửi cùng lịch sử trước đó.
        messages = history + [
            {
                "role": "user",
                "content": user_msg,
        ...
        for chunk in stream:
            # Chunk cuối có thể có content=None.
            delta = chunk.choices[0].delta.content or ""
        ...
        # Chỉ cập nhật history sau khi đã nhận đủ phản hồi.
        history.append(
            {
                "role": "user",
                "content": user_msg,
        ...
        # Một lượt gồm 2 message:
        # user + assistant.
        # Bốn lượt gần nhất tương ứng tối đa 8 message.
        history = history[-8:]
...
def retry_with_backoff(
    ...
    """
    Gọi fn(). Nếu xảy ra exception, thử lại theo exponential backoff.

    Tổng số lần gọi tối đa:
        1 lần đầu + max_retries lần thử lại.

    Delay trước mỗi lần thử lại:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # attempt == max_retries nghĩa là đã dùng hết
            # số lần retry được phép.
...
def run_assistant(
    ...
    Trợ lý CLI hoàn chỉnh:
        - Sử dụng persona làm system prompt.
        - Gọi API theo dạng streaming.
        - Retry khi lời gọi API lỗi.
        - Giữ tối đa 4 lượt hội thoại gần nhất.
        - Thống kê số lượt, token và chi phí.
    ...
    while True:
        # Phải kiểm tra trước get_input().
        # Điều này tránh đọc dư input khi max_turns đã đạt giới hạn.
        if max_turns is not None and turns >= max_turns:
            break

        user_msg = get_input()

        # Kết thúc không phân biệt chữ hoa, chữ thường và khoảng trắng.
        if user_msg.strip().lower() in ("quit", "exit", "bye"):
            break

        # System prompt luôn đứng đầu mỗi lời gọi.
        messages = (
            [
                {
                    "role": "system",
        ...
        # Xuống dòng sau khi nhận hết response.
        print()

        # Lưu lượt hội thoại vừa hoàn thành.
        history.append(
            {
                "role": "user",
                "content": user_msg,
        ...
        # Chỉ giữ 4 lượt gần nhất:
        # 4 user messages + 4 assistant messages = 8 messages.
        history = history[-8:]
        ...
        tokens_used += (
            count_tokens(user_msg, OPENAI_MODEL)
            + count_tokens(reply, OPENAI_MODEL)
        )
